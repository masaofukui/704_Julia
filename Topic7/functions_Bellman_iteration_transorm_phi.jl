...
function Euler_iteration_once(param,c_pol_old, param_changed)
    ...
    for (ia,a_future) in enumerate(ag)
        ...
        a_today_unconstrained[ia,:] =  (c_today_unconstrained .+ a_future .- yg .+ r.*phi )./(1.0 .+r) .+ zeros(eltype(a_today_unconstrained),Ny)
    end
    ...
    for (iy,y_today) in enumerate(yg)
        ...
        c_pol_new[:,iy] = (1 .+r).*ag .+ y_today .-r.*phi .- a_pol_new[:,iy]
    end
